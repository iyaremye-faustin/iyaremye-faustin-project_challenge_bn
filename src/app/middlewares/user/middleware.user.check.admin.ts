...
class CheckUserIsAdminMiddleware extends BaseMiddleware {
  ...
  protected async middleware(
    ...
    if (!token) {
      return this.responseHandler(res, this.UNAUTHORIZED_CODE, 'Not Authorized');
    }
    ...
    if (decoded.errors) {
      return this.responseHandler(res, this.UNAUTHORIZED_CODE, 'Invalid Authentication');
		...
		if (payload.role.role_name !=='administrator') {
			return this.responseHandler(res, this.UNAUTHORIZED_CODE, 'Only Administrator can perform the operation');
		}
		req.user = payload
    return next();
  }
